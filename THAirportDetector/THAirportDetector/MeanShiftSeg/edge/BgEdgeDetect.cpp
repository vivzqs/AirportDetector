...
void BgEdgeDetect::ComputeEdgeInfo(BgImage* cim, float* confMap, float *rank, int wait_begin, int wait_end)
{
	x_ = cim->x_;
	y_ = cim->y_;
...
#ifdef SHOW_PROGRESS
   if (!m_pWaitBar && wait_end>wait_begin)
   {
	   m_pWaitBar = new CWaitBar(_T("强度计算..."));
	   m_pWaitBar->SetRange(wait_begin, wait_end,_T("强度计算..."));
   }
#endif
...
#ifdef SHOW_PROGRESS
   if(m_pWaitBar)m_pWaitBar->SetPos(20);
#endif
...
#ifdef SHOW_PROGRESS
   if(m_pWaitBar)m_pWaitBar->SetPos(100);
   if(m_pWaitBar)
   {
	   delete m_pWaitBar;
	   m_pWaitBar = NULL;
...
#ifdef SHOW_PROGRESS
	  int pos =  int(20.0*(j-WL_)/(y_-2*WL_))/10*10;
	  if(pos == 10 || pos == 20 )
	  {
		  if(m_pWaitBar)
			m_pWaitBar->SetPos(pos);
	  }
#endif
...
void BgEdgeDetect::GaussDiffFilter(BgImage* cim, float* grx, float* gry, float* rezIm)
{
   ...
   double* sf = NULL; //smooth filter
   double* df = NULL; //diff filter
...
#ifdef SHOW_PROGRESS
	  int pos = 20 + int(80.0*(allnum-l-ir+1)/allnum)/10*10;
	  if(m_pWaitBar)m_pWaitBar->SetPos(pos);
#endif
