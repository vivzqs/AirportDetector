...
msImageProcessor::msImageProcessor( bool bWaitBar )
{
#ifdef SHOW_PROGRESS
	m_bWaitBar = bWaitBar;
	m_pWaitBar = NULL;
...
/*******************************************************/
///沈占锋2009.11.23增加最后一个参数

BOOL msImageProcessor::DefineImage(FILE*& fpSaveFile, byte *data_, imageType type, int height_, int width_,bool bAllowCache/* = true*/,bool *bHaveSave/* = false*/)
{
	...
	//沈占锋增加，存储或读取已存储，用于判断是否需要进行RGBtoLUV函数的调用

	TCHAR tstrCurrentPath[MAX_PATH];
	memset(tstrCurrentPath, 0, MAX_PATH);
	GetCurrentDirectory(MAX_PATH, tstrCurrentPath);
	//std::string std_tmp = tstrCurrentPath;
	//mbstowcs(tstrCurrentPath,strDBName,MAX_PATH);
	//char strDBName[MAX_PATH],strtmp[MAX_PATH];
	//TCHAR strDBName[MAX_PATH],strtmp[MAX_PATH];
	//wcstombs(strDBName,tstrCurrentPath,MAX_PATH);
	CString strDBName(tstrCurrentPath),strtmp;

	//strcat(tstrCurrentPath,"\\SegTmp");
	
	//wcscat(tstrCurrentPath,L"\\SegTmp");
	strDBName += "\\SegTmp";
#if defined(__WIN32__) || defined(_WIN32) || defined(_WINDOWS)//for window c++ compiler
	TCHAR stt[MAX_PATH];
	//mbstowcs (stt,strDBName,MAX_PATH);
	BOOL bSuccess = CreateDirectory(tstrCurrentPath,NULL);
//	strcpy(strDBName,tstrCurrentPath);
	//wcscpy(strDBName,tstrCurrentPath);
	strDBName = tstrCurrentPath;
#else              //for gcc
	_ASSERT(FALSE);
#endif
	//int nSuccess = mkdir(strDBName);
	//FILE *fpSaveFile;
	...
	for(int i=1;i<1000;i++)
	{
		...
		strtmp.Format(L"%03d",i);
		//sprintf(strtmp,"%03d",i);
		CString _strTmpDBName =strDBName + "\\SinSeg" + strtmp;
		//char strTmpDBName[MAX_PATH];
		//strcpy(strTmpDBName,strDBName);
		//strcat(strTmpDBName,"\\SinSeg");
		//strcat(strTmpDBName,strtmp);
		_strTmpDBName += ".db";
		//strcat(strTmpDBName,".db");
		USES_CONVERSION;
		char * strTmpDBName = T2A(_strTmpDBName);   
		if((_access(strTmpDBName,0)) == -1 )continue;
		
		//\\AfxGetApp()->WriteProfileString("SINCE2008","CurrnetDBFile",strDBName);
		//TCHAR appName[MAX_PATH],keyName[MAX_PATH],stringName[MAX_PATH]  ;//(LPCTSTR)strTmpDBName;
		//mbstowcs(appName,"SINCE2008",MAX_PATH);
		//mbstowcs(keyName,"CurrnetDBFile",MAX_PATH);
		//mbstowcs(stringName,strTmpDBName,MAX_PATH);

		//BOOL bSuc = WriteProfileString(appName,keyName,stringName);
		//bool bSuc = WriteProfileString((LPCTSTR)"SINCE2008",(LPCTSTR)"CurrnetDBFile",(LPSTR)strTmpDBName);
		//_ASSERT(bSuc);
		//fopen_s(&fpSaveFile,strTmpDBName,"rb");
		fpSaveFile = fopen(strTmpDBName,"rb");
		while(fpSaveFile)
		{
			*bHaveSave = true;
			int m_W,m_H,m_D;
			...
			int resu = fread(&m_W,(int)sizeof(int),1,fpSaveFile);
			//ASSERT(resu == 1);
			resu = fread(&m_H,(int)sizeof(int),1,fpSaveFile);
			//ASSERT(resu == 1);
			resu = fread(&m_D,sizeof(int),1,fpSaveFile);
			//ASSERT(resu == 1);
			if(m_D != dim || m_H != height_ || m_W != width_)
			{
				*bHaveSave = false;
				fclose(fpSaveFile);
				break;
			...
			unsigned char *m_dataRead = new unsigned char[m_W*m_D];
			resu = fread(m_dataRead,(int)sizeof(unsigned char)*m_W*m_D,1,fpSaveFile);
			//ASSERT(resu == 1);
			for (int i=0;i<m_W*m_D;i++)
			{
				if(m_dataRead[i] != data_[i])
				{
					*bHaveSave = false;
					//if(m_dataRead){delete m_dataRead;m_dataRead = NULL;}
					break;
				}
			}

			if(!(*bHaveSave)){delete m_dataRead;m_dataRead = NULL;fclose(fpSaveFile);break;}
			resu = fread(m_dataRead,(int)sizeof(unsigned char)*m_W*m_D,1,fpSaveFile);
			//ASSERT(resu == 1);
			for (int i=0;i<m_W*m_D;i++)
			{
				if(m_dataRead[i] != data_[i+m_W*m_H*m_D/2])
				{
					*bHaveSave = false;
					//if(m_dataRead){delete m_dataRead;m_dataRead = NULL;}
					break;
			...
			for (int i=0;i<m_W*m_D;i++)
			{
				if(m_dataRead[i] != data_[i+m_W*(m_H-1)*m_D])
				{
					*bHaveSave = false;
					//if(m_dataRead){delete m_dataRead;m_dataRead = NULL;}
					break;
			...
			delete m_dataRead;
			if(!(*bHaveSave))
			{m_dataRead = NULL;fclose(fpSaveFile);break;}
			//已经存储且相符
			resu = fread(luv, sizeof(float)*m_W*m_H*m_D,1,fpSaveFile);
			//ASSERT(resu == 1);
			//fclose(fpSaveFile);
			break;//不允许其循环
		}
	...
	if ( !(*bHaveSave))//已经存储且相符
	{
#ifdef SHOW_PROGRESS
		if(!m_pWaitBar && m_bWaitBar)
		{
			m_pWaitBar = new CWaitBar(_T("颜色转换..."));	
			m_pWaitBar->SetRange(m_wait_begin, m_wait_end, _T("颜色转换..."));
		}
#endif
		if(dim == 1)
		{
			...
			{
#ifdef SHOW_PROGRESS
				int per = (int)(100.0*i/height_/width_ + 0.5);
				if(per % 10 == 0 && m_pWaitBar)m_pWaitBar->SetPos(per);
#endif
				luv[i]	= (float)(data_[i]);
			}
		}
		...
		if(bAllowCache)
		{
			...
			//char strTmpDBName[MAX_PATH];	
			char* strTmpDBName;
			CString _strTmpDBName;
			for(int i=1;i<1000;i++)
			{
				strtmp.Format(L"%03d",i);
				//sprintf(strtmp,"%03d",i);
				_strTmpDBName = strDBName + "\\SinSeg" + strtmp;
				//strcpy(strTmpDBName,strDBName);
				//strcat(strTmpDBName,"\\SinSeg");
				//strcat(strTmpDBName,strtmp);
				_strTmpDBName += ".db";
				...
				USES_CONVERSION;
				strTmpDBName = T2A(_strTmpDBName);
				if((_access(strTmpDBName,0)) == -1 )
					break;
			}
			//\\AfxGetApp()->WriteProfileString("SINCE2008","CurrnetDBFile",strTmpDBName);
			...
			//bool bSuc = WriteProfileString((LPCTSTR)"SINCE2008",(LPCTSTR)"CurrnetDBFile",(LPSTR)strTmpDBName);
			fopen_s(&fpSaveFile,strTmpDBName,"wb");
			//ASSERT(fp);
			//\\filep.Open(strTmpDBName,CFile::typeBinary|CFile::modeWrite|CFile::modeCreate);

			//存储以下几个参数：width,height,dim,第一行，中间行，最后行（用于判断已经存储的数据是不是新的数据，如果是，可以跳过若干个步骤）
			int resu = fwrite(&width_,(int)sizeof(int),1,fpSaveFile);
			//ASSERT(resu == 1);
			//\\filep.Write(&width_,(int)sizeof(int));
			resu = fwrite(&height_,(int)sizeof(int),1,fpSaveFile);
			//ASSERT(resu == 1);
			//\\filep.Write(&height_,(int)sizeof(int));
			resu = fwrite(&dim,(int)sizeof(int),1,fpSaveFile);
			//ASSERT(resu == 1);
			//\\filep.Write(&dim,(int)sizeof(int));
			resu = fwrite(&(data_[0]),(int)sizeof(byte)*width_*dim,1,fpSaveFile);
			//ASSERT(resu == 1);
			//\\filep.Write(&(data_[0]),(int)sizeof(unsigned char)*width_*dim);
			resu = fwrite(&(data_[width_*height_*dim/2]),(int)sizeof(byte)*width_*dim,1,fpSaveFile);
			//ASSERT(resu == 1);
			//\\filep.Write(&(data_[width_*height_*dim/2]),(int)sizeof(unsigned char)*width_*dim);
			resu = fwrite(&(data_[width_*(height_-1)*dim]),(int)sizeof(byte)*width_*dim,1,fpSaveFile);
			//ASSERT(resu == 1);
			//\\filep.Write(&(data_[width_*(height_-1)*dim]),(int)sizeof(unsigned char)*width_*dim);

			resu = fwrite(luv,(int)sizeof(float)*width_*height_*dim,1,fpSaveFile);
			//ASSERT(resu == 1);
			//\\filep.Write(luv,(int)sizeof(float)*width_*height_*dim);
...
BOOL msImageProcessor::DefineBgImage(byte* data_, imageType type, int height_, int width_)
{
	//obtain image dimension from image type
	int dim;
	if(type == COLOR)
...
void msImageProcessor::Filter(int sigmaS, float sigmaR, SpeedUpLevel speedUpLevel)
{
	...
	*/
////shenzf 20100303,把原来在InitializeOutput( void )中的初始化的变量移至此
...
int  msImageProcessor::GetRegions(int**labels_out, unsigned char**averageRGB_out, int**MPC_out)
{
	//check to see if output has been defined for the given input image...
	if(class_state.OUTPUT_DEFINED == false)
		return -1;
	...
	if(!(*averageRGB_out	= new unsigned char [regionCount*N]))
	{
		ErrorHandler("msImageProcessor", "GetRegions", "Not enough memory.");
		return -1;
	}
	...
	//populate modes_out and MPC_out with the color and point
	//count of each region
	byte *tmpRGB = new byte[regionCount*N+2];
	for (i = 0;i < regionCount; i++)
		LUVtoRGB(&modes[i],&tmpRGB[i]);
	for(i = 0; i < regionCount*N; i++)
		(*averageRGB_out)[i]	= tmpRGB[i];
	delete []tmpRGB;
	for(i = 0; i < regionCount; i++)
...
void msImageProcessor::NonOptimizedFilter(float sigmaS, float sigmaR)
{
	...
	if(m_pWaitBar==NULL && m_bWaitBar)
	{
		m_pWaitBar = new CWaitBar(_T("图像滤波..."));
		m_pWaitBar->SetRange(m_wait_begin, m_wait_end,_T("图像滤波...")) ;
	}
	...
	for(i = 0; i < L; i++)
	{
		...
		int per = (int)(percent_complete + 0.5);
		if(m_pWaitBar)m_pWaitBar->SetPos(per);
...
void msImageProcessor::BuildRAM( void )
{
	...
	//FILE *fp;
	//fopen_s(&fp,"d:\\56.txt","w");
	//for (int i=0;i<NODE_MULTIPLE*regionCount;i++)
	//{

	//	fprintf(fp,"%d %d %f\r\n",raPool[i].label,raPool[i].edgePixelCount,raPool[i].edgeStrength);
	//}
	//fclose(fp);
	//delete []raPool;
	//raPool = NULL;
...
void msImageProcessor::NewOptimizedFilter1(float sigmaS, float sigmaR)
{
	...
	for(i = 0; i < L; i++)
	{
		...
		percent_complete = (float)(i/(float)(L))*100;
		msSys.Prompt("\r%2d%%", (int)(percent_complete + 0.5));
		int per = (int)(percent_complete + 0.005);
		if(m_pWaitBar && i%50 == 0)m_pWaitBar->SetPos(per);
#endif
